...
def main():
    ...
    for epoch in range(start_epoch, args.epochs):
        try:
            ...
            train_loss = train_epoch(model, train_loader, optimizer, noise_scheduler, device, scaler, epoch, args)
            logger.info(f"Эпоха {epoch+1}/{args.epochs}, Потери при обучении: {train_loss:.6f}")
            
            # Очищаем память GPU
            memory_cleanup()
            
            # Валидируем модель
            val_loss, val_psnr = validate(model, val_loader, noise_scheduler, device, epoch, args)
            logger.info(f"Эпоха {epoch+1}/{args.epochs}, Потери при валидации: {val_loss:.6f}, PSNR: {val_psnr:.2f} dB")
            
            # Очищаем память GPU
            memory_cleanup()
            
            # Визуализируем результаты каждые 5 эпох или на последней эпохе
            if (epoch + 1) % 5 == 0 or epoch == args.epochs - 1:
                visualize_results(model, val_loader, noise_scheduler, device, epoch, args)
                memory_cleanup()
            ...
            # Сохраняем модель через указанные интервалы или если это лучшая модель
            if (epoch + 1) % args.save_interval == 0 or epoch == args.epochs - 1 or is_best:
                save_checkpoint(model, optimizer, epoch, val_loss, val_psnr, args, is_best)
            
        except Exception as e:
            logger.error(f"Ошибка во время обучения на эпохе {epoch+1}: {str(e)}")
            logger.exception(e)
            # Сохраняем аварийный чекпоинт
            save_checkpoint(model, optimizer, epoch, float('inf'), 0, args, is_best=False)
            break
